...
function renderStats (rows, commits) {
  ...
  const fileLengths = d3.rollups(rows, v => d3.max(v, d => d.line), d => d.file);
  stat('Average file length', d3.mean(fileLengths, d => d[1]).toFixed(1));

  const longest = d3.greatest(rows, d => d.length);
  stat('Longest line (chars)', longest.length);
}
...
function renderScatter (commits) {
  ...
  const y = d3.scaleLinear().domain([0, 24]).range([usable.y0, usable.y1]);
...
function renderCount (n) {
  d3.select('#selection-count')
    .text(n ? `${n} commit${n > 1 ? 's' : ''} selected`
            : 'No commits selected');
}
...
function renderLangBreakdown (commits) {
  ...
  breakdown.sort((a, b) => d3.descending(a[1], b[1])).forEach(([lang, count]) => {
    ...
    out.append('dd').text(`${count} lines (${d3.format('.1%')(count / total)})`);
  });
}
